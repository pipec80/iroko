...
-- ============================================================================
-- Mercado Pago Chile catalog
-- ============================================================================
-- Preserve internal plan identities while exposing the approved customer-facing
-- Free / Plus / Pro names. These CLP monthly entries drive pending
-- preapprovals without a reusable Mercado Pago plan or external price id.
...
-- ============================================================================
-- Durable hosted-checkout coordination
-- ============================================================================
...
BEGIN
  WITH candidates AS (
    SELECT customer.account_id, subscription.plan_id, subscription.provider,
      subscription.external_subscription_id, subscription.created_at,
      row_number() OVER (
  ...
  SELECT account_id, plan_id, provider,
    CASE WHEN position = 1 THEN 'needs_review' ELSE 'failed' END,
    external_subscription_id,
    CASE WHEN position = 1 THEN 'preexisting_incomplete_requires_review'
      ELSE 'preexisting_duplicate_incomplete' END,
    created_at
  FROM candidates
...
BEGIN
  ...
  INSERT INTO billing.subscriptions (
    ...
    WHERE external_subscription_id IS NOT NULL DO NOTHING;
  SELECT status INTO v_subscription_status FROM billing.subscriptions
  ...
  SET external_subscription_id = btrim(p_external_subscription_id),
    checkout_url = COALESCE(p_checkout_url, checkout_url), lease_expires_at = NULL,
    failure_code = NULL,
    status = CASE
...
CREATE TRIGGER sync_billing_checkout_intent
  AFTER INSERT OR UPDATE OF status, external_subscription_id ON billing.subscriptions
