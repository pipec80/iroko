...
| Migration parity          | Local ↔ linked Supabase                                            | `supabase migration list --linked` on 2026-09-10: the four `20260909*` billing migrations were local-only and were applied with `supabase db push`. Parity holds as of that push; re-check after any later `supabase/migrations/` merge (CI/CD does not push). | 2026-09-10        | Change-bound | CURRENT until the next migration merge                             |
...
| Billing providers         | Mercado Pago reference; Stripe, Paddle, Lemon Squeezy              | Mercado Pago **sandbox** checkout → active → paid → cancel exercised against production 2026-09-10 (informal, not captured as PR evidence); adapter fix #179. Formal certification, the reconciliation worker schedule, and Stripe/Paddle/Lemon remain open.   | 2026-09-10        | Change-bound | Sandbox circuit observed; formal certification **[NO VERIFICADO]** |
