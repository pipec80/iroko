...
Last updated: **2026-09-23** (first real reconciliation-discovery scans, Cloud
...
| Reconciliation discovery         | Supabase scheduler → Vercel Node, live subscription | The first real discovery scans (2026-09-22/23) failed hourly: first `provider_fetch_failed` (Mercado Pago rejects `limit` above 15; fixed in #205), then, at the 2026-09-23 22:00 UTC scan, `reducer_failed` (discovered invoices carried the checkout id as account; fixed in #208). `failure_count` is at its cap of 10 and `last_completed_at` is empty; the worker still answered HTTP 200 each hour.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | 2026-09-23 22:12 UTC | Change-bound                     | **FAILED** until a scan completes after #208 is deployed ([Plan 011 matrix](../exec-plans/active/011-mercadopago-v1-chile-acceptance.md), MP-12) |
...
| Failing scan diagnosis             | Replaying discovery with the fixed provider returned one `invoice_paid` event whose `accountId` was the checkout id, explaining `reducer_failed`.                                                                                                                                          | The replay used the seller token read-only; the fix is #208.                                                        |
