...
| AUD-024 | No automated Cloud check confirmed `/sentry-tunnel` still reaches Sentry ingest | P1       | Resolved by PR TBD (2026-08-11)    | 007  | Completed |
...
- **add a Cloud smoke check for Supabase workers and provider callbacks.** The Sentry tunnel portion of this item is resolved — see AUD-024 below. The email worker and provider webhook portions remain open, tracked as AUD-025 (wire `private.email_worker_health()`, already returning real HTTP status per Plan 002, into an automated nightly gate instead of manual checks) and AUD-026 (a Stripe/MercadoPago webhook smoke probe — requires its own explicit approval before design, since it touches real payment providers). Neither is part of this pass;
