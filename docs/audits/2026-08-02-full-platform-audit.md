...
| AUD-025 | Email worker health (`private.email_worker_health()`) was checked manually only, no automated Cloud gate | P1       | Resolved by PR TBD (2026-08-11)    | 008  | Completed |
...
- **add a Cloud smoke check for the Sentry tunnel and provider webhook callbacks.** The email worker portion of this item is resolved — see AUD-025 below. Sentry's tunnel has already broken silently once before (AUD-005 — next-intl's locale proxy redirected the POST and dropped every event, again with nothing surfacing the failure on its own); provider webhook callbacks (Stripe, MercadoPago) are the same shape of risk — if signature verification or the endpoint URL breaks, a payment or cancellation stops reflecting in the account with no visible error, just a customer finding out first. Both remain open, tracked separately (Sentry tunnel: AUD-024; provider webhooks: AUD-026, blocked on a real, non-mock payment provider existing to smoke-test — none is configured today);
