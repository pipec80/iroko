...
| AUD-024 | No automated Cloud check confirmed `/sentry-tunnel` still reaches Sentry ingest                          | P1       | Resolved by PR #113 (2026-08-11)   | 007  | Completed |
| AUD-025 | Email worker health (`private.email_worker_health()`) was checked manually only, no automated Cloud gate | P1       | Resolved by PR #114 (2026-08-11)   | 008  | Completed |
...
### AUD-024 — No Cloud smoke check for the Sentry tunnel (Resolved 2026-08-11)

`src/test/e2e/sentry-tunnel.spec.ts` only asserted that `/sentry-tunnel` is not 307-redirected by the proxy (the AUD-005 regression itself). Nothing proved an event actually survives the full round trip to Sentry's real ingest in production — a different failure (DSN misconfiguration, a Sentry-side rejection, or a future change to the tunnel/proxy interaction) could still silently drop every browser-side event with no automated signal, the same failure mode that let AUD-005 go unnoticed for weeks.

**Closure evidence (2026-08-11):** a second, `@smoke`-tagged test in the same spec sends a minimal valid Sentry envelope to `/sentry-tunnel?o=<orgId>&p=<projectId>[&r=<region>]` (the exact query-param shape the browser SDK itself builds from the DSN), asserting Sentry's own `200` response. No production code changed. Guarded to run only against a deployed target (`nightly.yml`, production) via the same `PLAYWRIGHT_BASE_URL` pattern `analytics.spec.ts`/`onboarding.spec.ts` already use, inverted — it never runs against local Supabase in `ci.yml`, so it doesn't spend Sentry's free-tier error quota on every PR. The event carries a stable fingerprint (`sentry-tunnel-cloud-smoke-check`) so nightly runs fold into one recurring issue instead of growing the backlog.

**Self-correction during implementation:** the first version of this check assumed the tunnel route is a generic relay that reads `dsn` from the envelope body — wrong. A bare `POST /sentry-tunnel` with no query string got a real `404` against production. Before concluding the tunnel was broken, this was root-caused rather than patched around: reproduced the identical 404 in a fully local `pnpm build && pnpm start` (ruling out anything Vercel-specific), isolated it with a control probe against a sibling rewrite in the same `next.config.ts` (PostHog's `/ingest/static/*`, added by PR #109), which worked fine (`200`) both locally and in production — proving the app's own rewrite composition is healthy — and then read the exact installed `@sentry/nextjs@10.67.0` source (`config/withSentryConfig/tunnel.ts`) to find the real matching rule: the injected rewrite only matches `?o=<orgId>&p=<projectId>[&r=<region>]`. There was no production regression; the check's own request was shaped wrong. Fixed and reverified live: `nightly.yml` run `31514594343` passed 17/17, and the resulting event landed in Sentry as issue `IROKO-B`. See `docs/exec-plans/completed/007-cloud-smoke-sentry-tunnel.md` and `docs/runbooks/sentry-tunnel-smoke.md`.

### AUD-025 — No automated Cloud gate on email worker health (Resolved 2026-08-11)

`private.email_worker_health()` (Plan 002) already exposes the real HTTP status of the last worker invocation, but nothing ran it automatically — it was a manual step in `docs/runbooks/email-queue.md`. The same "cron reports success but nobody checks the real outcome" risk this audit's own hardening section calls out for Sentry/webhooks applied here too, just with the detection mechanism already built and simply not wired to anything periodic.

**Closure evidence (2026-08-11):** new `nightly.yml` job `email-worker-health` calls `private.email_worker_health()` through a new `service_role`-only wrapper (`public.get_email_worker_health()`, `supabase/migrations/20260811200000_email_worker_health_rpc.sql`) and fails if `status_code` isn't 2xx, `timed_out` is true, or `invoked_at` is older than 300s. `private` isn't in PostgREST's exposed schema list, so a direct call wasn't possible — rather than adding a new, broadly-scoped secret (a Supabase management access token) just to run `supabase db query --linked` from CI, the wrapper reuses the existing `SUPABASE_SECRET_KEY`, following the same `SECURITY DEFINER`-wrapper pattern already used for `admin_list_accounts`/`get_platform_audit_logs`. Verified: local `pnpm supa:reset` + `supabase test db` (188/188 pgTAP tests pass, no regression), a direct REST call against the local instance confirms `anon` gets `401 permission denied` while `service_role` gets the real health row, and the migration is pushed and in parity on the linked Cloud project (`supabase migration list --linked`).

**Self-correction during implementation:** the first `workflow_dispatch` on the branch failed with a genuine `curl` connection error (exit 7) — `secrets.NEXT_PUBLIC_SUPABASE_URL` resolves to the local-dev placeholder (`http://127.0.0.1:54321`) at the repository-secret level; the real Cloud value only exists in the `production` GitHub Environment. Added `environment: production` to the job (the same construct `ci.yml`'s `build` job already uses). That surfaced a second, platform-level constraint: the `production` environment has a branch-protection deployment rule, so it only resolves on `main` — a feature-branch `workflow_dispatch` cannot exercise the real Cloud path at all, by GitHub's own design, not a bug in this change. Live end-to-end confirmation against Cloud is deferred to the first run on `main` after merge (manual dispatch or the 3am schedule) rather than faked or skipped silently.
...
- **add a Cloud smoke check for provider webhook callbacks.** The Sentry tunnel portion of this item is resolved (AUD-024) and the email worker portion is resolved (AUD-025). Provider webhook callbacks (Stripe, MercadoPago) remain open, tracked as AUD-026. **Unblocked as of 2026-09-10:** Mercado Pago sandbox credentials are configured in production and `POST /api/webhooks/mercadopago` was verified end to end manually, so a real (non-`mock`) provider now exists to smoke-test. Designing the automated Cloud gate still needs its own approval since it touches a real payment provider; fold it into Phase 2 certification (`011-phase2-mercadopago-tasks.md`).
- ensure Vercel automation bypass is correctly configured for protected previews. **Revalidated 2026-08-11**: already confirmed working for its current single consumer — `nightly.yml`'s production smoke tests use `VERCEL_AUTOMATION_BYPASS_SECRET` correctly and the last 8 nightly runs are green. No protected-preview consumer exists in CI today (`ci.yml`'s `e2e`/`e2e-webkit` jobs build and test entirely against local Supabase, never touching a Vercel preview URL), so there is no active gap to close — this line stays only as a pointer if a future workflow starts targeting preview deployments directly;
...
| AUD-024     | #113 | manual `workflow_dispatch` of `nightly.yml` on the branch        | First dispatch (bare POST, wrong assumption about the tunnel mechanism) failed with a real `404`, root-caused (not dismissed) via a local `pnpm build`/`start` repro + a control probe against PostHog's sibling rewrite + reading the installed `@sentry/nextjs@10.67.0` source. After fixing the check's request shape (query params `o`/`p`/`r`, matching what the browser SDK builds), run `31514594343` passed 17/17; `search_issues` confirmed issue `IROKO-B` landed in Sentry with `firstSeen`/`lastSeen` matching the run | Claude Code | 2026-08-11 | Completed |
| AUD-025     | #114 | local `main` + local Supabase + linked Cloud + nightly on `main` | `supa:reset` + `supabase test db` (188/188 pgTAP pass); direct REST calls confirmed the `anon`/`service_role` gate; migration reached linked parity. Deferred Cloud validation was closed 2026-08-14: after correcting invalid production-environment secrets, `nightly.yml` ran 3/3 green and reported fresh HTTP 200 worker health. See Plan 008.                                                                                                                                                                                | Claude Code | 2026-08-14 | Completed |
