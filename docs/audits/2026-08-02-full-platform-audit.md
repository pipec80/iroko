...
- **add a Cloud smoke check for provider webhook callbacks.** The Sentry tunnel portion of this item is resolved (AUD-024) and the email worker portion is resolved (AUD-025). Provider webhook callbacks (Stripe, MercadoPago) remain open, tracked as AUD-026 — blocked on a real, non-mock payment provider existing to smoke-test (none is configured today, only the `mock` provider); requires its own explicit approval before design, since it would touch real payment providers;
...
| AUD-025     | #114 | local `main` + local Supabase + linked Cloud (migration only) | `supa:reset` + `supabase test db` (188/188 pgTAP pass); direct REST calls against the local instance confirm `anon` → `401 permission denied`, `service_role` → real health row; migration pushed and in parity on linked Cloud (`supabase migration list --linked`). End-to-end Cloud dispatch blocked by the `production` environment's branch-protection rule (feature branches can't resolve it) — deferred to first run on `main` post-merge                                                                                  | Claude Code | 2026-08-11 | In progress |
