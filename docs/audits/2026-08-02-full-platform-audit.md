...
**Closure evidence (2026-08-11):** a second, `@smoke`-tagged test in the same spec hand-builds a minimal valid Sentry envelope and POSTs it to `/sentry-tunnel`, asserting Sentry's own `200` response. The tunnel route `withSentryConfig` injects is a pure relay (reads `dsn` off the envelope, forwards the body verbatim, returns Sentry's response unmodified) — a `200` is Sentry's own confirmation, not an artifact of our code. No production code changed. Guarded to run only against a deployed target (`nightly.yml`, production) via the same `PLAYWRIGHT_BASE_URL` pattern `analytics.spec.ts`/`onboarding.spec.ts` already use, inverted — it never runs against local Supabase in `ci.yml`, so it doesn't spend Sentry's free-tier error quota on every PR. The event carries a stable fingerprint (`sentry-tunnel-cloud-smoke-check`) so nightly runs fold into one recurring issue instead of growing the backlog. See `docs/exec-plans/completed/007-cloud-smoke-sentry-tunnel.md` and `docs/runbooks/sentry-tunnel-smoke.md`.
...
| AUD-024     | TBD  | manual `workflow_dispatch` of `nightly.yml` on the branch | Pending: run `gh workflow run nightly.yml --ref feat/sentry-tunnel-cloud-smoke-check` and confirm the new `@smoke` test in `sentry-tunnel.spec.ts` gets a real `200` from Sentry before merge                                                                         | Claude Code | 2026-08-11 | In progress |
