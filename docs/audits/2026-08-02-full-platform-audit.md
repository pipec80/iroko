...
| AUD-025     | TBD  | local `main` + local Supabase | `supa:reset` + `supabase test db` (188/188 pgTAP pass); direct REST calls against the local instance confirm `anon` → `401 permission denied`, `service_role` → real health row. Cloud reverification pending a manual `workflow_dispatch` of `nightly.yml` on the branch before merge | Claude Code | 2026-08-11 | In progress |
