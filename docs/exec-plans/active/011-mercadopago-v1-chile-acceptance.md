...
`src/app/global-error.tsx`; el reintento fuera del sandbox pasó. El drill
combinado local de 25 filas está documentado, pero aún no se ha registrado su
ejecución. Esta evidencia prueba código y pruebas locales de MP-12/14, no
proveedor, Cloud, interrupción de proceso ni aceptación interna.
...
| MP-09 Refund total, contracargo y mediación                                      | [Reembolso de pago][mp-refund], [Webhooks][mp-webhooks]                                                                      | Implementado: `recoverResource` usa el pago fresco para clasificar refund total por importes normalizados y conserva `refunded`, `charged_back` e `in_mediation` probados aun sin importes válidos. El RPC idempotente persiste evidencia acotada; la rama de anomalía no llama al reducer y la resolución sigue siendo privada y manual. | Provider cubre refund total, refund por estado, contracargo y mediación; recovery verifica el RPC normalizado y la exclusión del reducer; SQL 37/41 cubren deduplicación, límites y conservación de acceso. Es evidencia local, no observación del proveedor ni ejecución del resolver. | Refund/contracargo/mediación reales, manejo de alerta y resolución manual **[NO VERIFICADO]**.                     | Fases 2/6: ejercer cada estado en proveedor, correlacionar la fila, comparar evidencia sanitizada según el runbook, observar una alerta accionable y ejecutar bajo autorización la resolución manual; confirmar que el acceso no cambia automáticamente.                                                    |
| MP-10 Reembolso parcial                                                          | [Reembolso parcial/total][mp-refund], [consulta de reembolso][mp-get-refund]                                                 | Implementado para jobs de recovery ya encolados: el pago fresco clasifica `partial_refund` solo con `0 < transaction_amount_refunded < transaction_amount`, importes enteros válidos y moneda de tres letras en mayúsculas; persiste evidencia acotada sin pasar por el reducer. Límite: un webhook `payment` ya vinculado y `approved` puede normalizarse como `invoice_paid` desde el authorized payment aunque el pago incluya importe reembolsado, sin encolar recovery ni ejecutar esta clasificación. | Provider cubre parcial, total, cero, CLP decimal, negativos y moneda ausente/malformada; recovery comprueba un único upsert de nueve argumentos y cero llamadas al reducer; SQL 41 cubre 17 aserciones de restricciones, idempotencia y suscripción sin cambios. Esta evidencia local cubre jobs ya encolados, no el ingreso ordinario vinculado. | Reembolso parcial real, manejo de alerta y ejecución del resolver manual **[NO VERIFICADO]**.                     | Gate de integración de código previo a aceptación operacional: enrutar por la clasificación de anomalías los ingresos pertinentes de webhook/discovery y agregar regresión para un pago vinculado `approved` con importe reembolsado. Luego observar un parcial real, comparar evidencia sanitizada, manejar la alerta y ejecutar la resolución manual autorizada. Proveedor/Cloud/manual **[NO VERIFICADO]**; no automatizar reembolsos desde Iroko. |
...
| MP-12 Factura omitida totalmente por webhooks                                    | [Búsqueda paginada de facturas por preapproval][mp-api]                                                                      | Implementado: discovery paginada y acotada por suscripción, cursor y watermark durables en `billing.reconciliation_state`, normalización de invoices y aplicación por el reducer compartido. | **Probado localmente — Plan 011d / Task 4, 2026-09-16:** pgTAP 559/559 y billing 216/216 cubren el estado durable, cursor/replay y el ingreso de eventos descubiertos sin depender de webhook/job conocido. Typecheck y lint también pasaron. | Omisión real de webhook/factura y convergencia contra Mercado Pago **[NO VERIFICADO]**.                            | 011e/011f: observar una invoice del proveedor completamente omitida, correlacionar aliases sanitizados, validar una sola aplicación y replay sin duplicados. Ejecutar además el drill local combinado antes de elevar evidencia operacional. |
...
| MP-14 Worker: fallos, avance e idempotencia                                      | [Consultas paginadas][mp-api]; límites locales del [diseño](../../architecture/mercadopago-reliability-design.md)            | Implementado: claims por `next_scan_at,subscription_id`, lotes ≤20, grupos de 5, presupuesto 45 s, cursor/watermark, backoff y aislamiento por candidato. El estado terminal se programa como skipped y el lease vencido se puede reclamar. | **Probado localmente — Plan 011d / Task 4, 2026-09-16:** pgTAP 559/559 cubre 25 filas, cursor, reclaimer y una segunda sesión `SKIP LOCKED`; billing 216/216 cubre fallo aislado, deadline y replay; ruta 7/7; typecheck y lint pasaron. | Invocaciones múltiples Cloud, interrupción de proceso y replay operacional **[NO VERIFICADO]**.                    | 011e/011f: ejecutar workers manuales y agendados, registrar más de un lote y una interrupción/reinicio reales. Antes, ejecutar el drill local documentado: 25 filas, una falla, cursor intermedio, lease expirado y comparación de IDs de invoice/event/payment. |
