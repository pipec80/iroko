...
| 6   | `X-Idempotency-Key` en `POST /preapproval` y `payer_email` con cuentas reales.                | **[NO VERIFICADO]**, sin cambios. El CLI oficial acepta `--idempotency-key` en POST/PUT, pero no hay confirmación para `/preapproval`; probarlo crea preapprovals reales y no se ejecutó sin autorización. `payer_email` toma el correo de la cuenta Iroko: con usuarios reales de Mercado Pago solo puede confirmarse con una cuenta real.                                                     |
...
| 8   | El build de CI falla de forma transitoria al descargar Google Fonts.                          | Ver PR aparte (tipografías locales).                                                                                                                                                                                                                                                                                                                                                            |
