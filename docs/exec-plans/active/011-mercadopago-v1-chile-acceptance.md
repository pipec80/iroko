...
| MP-10 Reembolso parcial                                                          | [Reembolso parcial/total][mp-refund], [consulta de reembolso][mp-get-refund]                                                 | Implementado: el pago fresco clasifica `partial_refund` solo con `0 < transaction_amount_refunded < transaction_amount`, importes enteros válidos y moneda de tres letras en mayúsculas. Persiste importes/moneda acotados en la anomalía abierta sin iniciar reembolsos ni pasar por el reducer. | Provider cubre parcial, total, cero, CLP decimal, negativos y moneda ausente/malformada; recovery comprueba un único upsert de nueve argumentos y cero llamadas al reducer; SQL 41 cubre 17 aserciones de restricciones, idempotencia y suscripción sin cambios. Es evidencia local. | Reembolso parcial real, manejo de alerta y ejecución del resolver manual **[NO VERIFICADO]**.                     | Fases 2/6: observar un parcial real, confirmar payload/estado e importes, comparar el registro sanitizado con Mercado Pago, observar la alerta y ejecutar la resolución manual autorizada; no duplicar, marcar como total ni automatizar un reembolso desde Iroko.                                             |
