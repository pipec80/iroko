...
## Verified provider behaviors (2026-09-23)

Behaviors of the Mercado Pago API that the official documentation does not
state (or states differently) and that shaped the code. Each was measured
against the live API with the test seller; none required a charge. Re-measure
one when its code path changes or when the provider announces an API change.

| Behavior                                                                                                                                                                      | How it was measured                                                                                    | Consequence in Iroko                                                                                                                                                |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GET /authorized_payments/search` rejects `limit` above 15 with `400 Invalid value for limit`.                                                                                | `limit` 1–15 accepted; 16–20, 25 and 50 rejected, on one preapproval.                                  | Discovery page size is capped at 15 (`MAX_AUTHORIZED_PAYMENTS_PAGE_SIZE`). Twenty made every scan fail until #205.                                                  |
| `POST /preapproval` ignores `X-Idempotency-Key`.                                                                                                                              | Two identical POSTs with the same key created two preapprovals; a third without a key created a third. | No header is sent. The local reservation (`reserve_billing_checkout`) and `needs_review` are the only duplicate protection.                                         |
| `POST /preapproval` requires `payer_email`.                                                                                                                                   | A POST without it answered `400 payer_email is required`.                                              | It is always sent, taken from the account owner. A payer using a different Mercado Pago account may be rejected (real accounts **[NO VERIFICADO]**).                |
| `GET /preapproval/search?external_reference=` does not filter, and a second immediate call answered `429`.                                                                    | The search for one checkout returned all 13 preapprovals of the seller; only one matched.              | Operators filter results client-side by `external_reference` (the checkout intent id) and pause between calls. Never trust the count.                               |
| The preapproval's `external_reference` is the checkout intent id, and the authorized payments repeat it.                                                                      | Compared a live preapproval with `billing.checkout_intents`.                                           | Every provider-derived event must resolve the reference to an account (webhook and recovery do; reconciliation uses the claimed subscription's account since #208). |
| Refunds created through the API with the test seller's credentials fail with `ERROR_UNAUTHORIZED: Unauthorized use of live credentials`; the seller panel shows no operation. | `mpcli refunds create` on an approved payment; `refunds list` stayed empty.                            | MP-09/10 need a real account. Iroko never issues refunds; it only ingests them.                                                                                     |
| Subscriptions are tested with the **production** credentials of a test account.                                                                                               | Official credentials documentation: test credentials exist only for Checkout API and Checkout Bricks.  | The test seller's `APP_USR` token is the only credential; the developer panel of a test user hides "test credentials".                                              |
| The official quality measurement does not apply to Subscriptions.                                                                                                             | The quality checklist tool answered `Product not homologable`.                                         | No official score exists; the acceptance matrix is internal certification only.                                                                                     |
